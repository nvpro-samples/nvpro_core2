...
// Information to exchange in shader
...
// Value is internal, should not displayed
...
{
public:
  TempCommandBuffer(VkDevice device, nvvk::QueueInfo queueInfo)
      : device(device)
      , queueInfo(queueInfo)
  {
    cmdPool = nvvk::createTransientCommandPool(device, queueInfo.familyIndex);
    NVVK_CHECK(nvvk::beginSingleTimeCommands(cmdBuff, device, cmdPool));
  }
  ~TempCommandBuffer() { assert((cmdBuff == VK_NULL_HANDLE) && "submitAndWait wasn't called"); }

  void submitAndWait()
  {
    assert(cmdBuff != VK_NULL_HANDLE);
    NVVK_CHECK(nvvk::endSingleTimeCommands(cmdBuff, device, cmdPool, queueInfo.queue));
    vkDestroyCommandPool(device, cmdPool, nullptr);
    cmdBuff = VK_NULL_HANDLE;
  }
  operator VkCommandBuffer() const { return cmdBuff; };
...
#define MAT_TO_INTERNAL_CASE(type_, internalType_, dims_)                                                              \
  ...
  bool updateImageFormat(uint32_t index, const std::vector<nvapp::ElementInspectorInternal::ValueFormat>& newFormat);
  ...
  bool updateBufferFormat(uint32_t index, const std::vector<nvapp::ElementInspectorInternal::ValueFormat>& newFormat);
  ...
  bool updateComputeFormat(uint32_t index, const std::vector<nvapp::ElementInspectorInternal::ValueFormat>& newFormat);
  ...
  struct InspectedBuffer
  {
    ...
    std::filesystem::path                              snapshotFileName;
  ...
  struct InspectedImage : public InspectedBuffer
  {
    ...
    InspectedImage& operator=(const InspectedImage& rhs)
    {
      ...
      {
        TempCommandBuffer cmd(device, queueInfo);

        //        image = allocator->createImage(cmd, rhs.snapshotContents.size(), rhs.snapshotContents.data(), createInfo);
        allocator->createImage(image, createInfo, DEFAULT_VkImageViewCreateInfo);
        uploader.appendImage(image, std::span(rhs.snapshotContents));
        uploader.cmdUploadAppended(cmd);
        cmd.submitAndWait();
      }
      uploader.deinit();

      //VkImageViewCreateInfo viewCreateInfo = DEFAULT_VkImageViewCreateInfo;
      ...
      imguiImage = ImGui_ImplVulkan_AddTexture(sampler, image.descriptor.imageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
      return *this;
    }
  };
  ...
  template <typename T>
  void setSnapshotFileNames(std::vector<T>& elements, const std::filesystem::path& stem)
  {
    for(auto& element : elements)
    {
      if(element.hasSnapshotContents)
      {
        element.snapshotFileName = stem;
        element.snapshotFileName += std::string("_") + element.name + ".csv";
  ...
  void requestFullSnapshotSave()
  {
    ...
    const std::filesystem::path stem = filename.stem();  // Without extension

    setSnapshotFileNames(m_inspectedImages, stem);
    setSnapshotFileNames(m_inspectedImagesCopies, stem);

    setSnapshotFileNames(m_inspectedBuffers, stem);
    setSnapshotFileNames(m_inspectedBuffersCopies, stem);
    setSnapshotFileNames(m_inspectedComputeVariables, stem);
    setSnapshotFileNames(m_inspectedComputeVariablesCopies, stem);

    setSnapshotFileNames(m_inspectedFragmentVariables, stem);
    setSnapshotFileNames(m_inspectedFragmentVariablesCopies, stem);
...
template <typename T>
bool updateFormat(T& inspection, const std::vector<nvapp::ElementInspectorInternal::ValueFormat>& newFormat)
{
  uint32_t originalSize = inspection.formatSizeInBytes;
  uint32_t newSize      = nvapp::ElementInspectorInternal::computeFormatSizeInBytes(newFormat);
...
void ElementInspector::onAttach(nvapp::Application* app)
{
...
template <typename T>
void imguiCopy(nvapp::Application* app, T& src, const std::vector<T>& existingOriginals, std::vector<T>& copies)
{
...
void imguiElementShowImageButtons(std::vector<T>& elements, std::vector<T>& elementsCopies)
{
  for(size_t elementIndex = 0; elementIndex < elements.size(); elementIndex++)
  {
    ...
    if(ImGui::ImageButton(fmt::format("##ImageButton{}", elementIndex).c_str(),
                          (ImTextureID)elements[elementIndex].imguiImage, ImVec2(SQUARE_BUTTON_SIZE, SQUARE_BUTTON_SIZE)))
    {
      elements[elementIndex].show = !elements[elementIndex].show;
    }
  ...
  if(!elementsCopies.empty())
  {
    ...
    for(size_t elementIndex = 0; elementIndex < elementsCopies.size(); elementIndex++)
    {
      ...
      if(ImGui::ImageButton(fmt::format("##ImageButtonCopy{}", elementIndex).c_str(),
                            (ImTextureID)elementsCopies[elementIndex].imguiImage, ImVec2(SQUARE_BUTTON_SIZE, SQUARE_BUTTON_SIZE)))
      {
        elementsCopies[elementIndex].show = !elementsCopies[elementIndex].show;
      }
      if(ImGui::IsItemHovered())
...
void ElementInspectorInternal::imGuiComputeVariable(uint32_t i, bool isCopy)
{
  ...
  {
    computeVar.mappedContents = (uint8_t*)computeVar.hostBuffer.mapping;
...
void ElementInspectorInternal::imGuiImage(uint32_t imageIndex, ImVec2& imageSize, bool isCopy)
{
  ...
  if(img.snapshotRequested)
  {
    ...
    img.mappedContents        = (uint8_t*)img.hostBuffer.mapping;
  ...
  if(ImGui::CollapsingHeader(img.name.c_str(), ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    if(img.isAllocated)
    {
      ...
      else
      {
        ...
        ImGui::Image((ImTextureID)img.imguiImage, regionSize);
        ...
        if(ImGui::IsItemHovered())
        {
          ...
          if(ImGui::BeginTooltip())
          {
            ...
            const uint8_t* contents = static_cast<uint8_t*>(img.hostBuffer.mapping);
            uint32_t       pixelIndex =
                static_cast<uint32_t>(center.x) + static_cast<uint32_t>(imageSize.x) * static_cast<uint32_t>(center.y);
            ...
            ImGui::Image((ImTextureID)img.imguiImage, ImVec2(SQUARE_BUTTON_SIZE, SQUARE_BUTTON_SIZE), uv0, uv1);
  ...
  for(uint32_t i = begin; i < end; i++)
  {
    ...
    }
    {
      ...
      if(scrollToItem == ~0u)
      {
        ...
        {
          uint32_t flash =
              uint32_t((buf.selectedFlashTimer.getMilliseconds() / SELECTED_FLASH_DURATION) * (2 * SELECTED_FLASH_COUNT + 1));
          if(flash % 2 == 0)
          {
      ...
      {
        if(buf.selectedFlashTimer.getMilliseconds() > SELECTED_FLASH_DURATION)
        {
          buf.selectedRow = ~0u;
        }
      }
                                           ...
                                           bool             showCollapsibleHeader /*= true*/)
{
  ...
  if(buf.snapshotRequested && buf.isAllocated && buf.isInspected)
  {
    ...
    buf.mappedContents        = (uint8_t*)buf.hostBuffer.mapping;
        ...
        nvutils::parallel_batches(
            ...
            [&](uint64_t i) {
              if(timeoutTimer.getSeconds() > m_settings.filterTimeoutInSeconds)
              {
                ...
                {
                  LOGW("Inspector filter timeout - consider reducing the buffer filtering range or increasing timeout (at the expense of interactivity)\n");
...
{
  m_alloc->destroyBuffer(inspectedBuffer.hostBuffer);
...
void ElementInspectorInternal::init(const ElementInspector::InitInfo& info)
{
  ...
  if(info.queueInfo.familyIndex == ~0u)
  {
    LOGE("ElementInspector: queue info cannot be ~0u\n");
    assert(false);
    return;
  }
...
void ElementInspectorInternal::initImageInspection(uint32_t index, const ElementInspector::ImageInspectionInfo& info)
{
  ...
  inspectedImage.queueInfo = m_queueInfo;
  ...
  if(inspectedImage.isAllocated)
  {
    ...
    m_alloc->destroyImage(inspectedImage.image);
    m_alloc->destroyBuffer(inspectedImage.hostBuffer);
  ...
  m_alloc->createImage(inspectedImage.image, info.createInfo);
  inspectedImage.createInfo = info.createInfo;

  {
    TempCommandBuffer cmd(m_device, m_queueInfo);
    nvvk::cmdImageMemoryBarrier(cmd, {inspectedImage.image.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL});
    ...
    nvvk::cmdImageMemoryBarrier(cmd, {inspectedImage.image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL});
    cmd.submitAndWait();
  }

  VkImageViewCreateInfo viewCreateInfo = DEFAULT_VkImageViewCreateInfo;
  viewCreateInfo.image                 = inspectedImage.image.image;
  viewCreateInfo.viewType              = VK_IMAGE_VIEW_TYPE_2D;
  viewCreateInfo.format                = info.createInfo.format;
...
bool ElementInspector::updateImageFormat(uint32_t index, const std::vector<nvapp::ElementInspector::ValueFormat>& newFormat)
{
  return m_internals->updateImageFormat(index, ElementInspectorInternal::toInternalFormat(newFormat));
}

bool ElementInspectorInternal::updateImageFormat(uint32_t index, const std::vector<nvapp::ElementInspectorInternal::ValueFormat>& newFormat)
{
  if(!m_isInitialized)
  {
    return false;
...
void ElementInspectorInternal::inspectImage(VkCommandBuffer cmd, uint32_t index, VkImageLayout currentLayout)
{
  ...
  nvvk::cmdImageMemoryBarrier(cmd, {internalImg.sourceImage, currentLayout, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL});
  nvvk::cmdImageMemoryBarrier(cmd, {internalImg.image.image, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
  ...
  nvvk::cmdImageMemoryBarrier(cmd, {internalImg.sourceImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, currentLayout});
  nvvk::cmdImageMemoryBarrier(cmd, {internalImg.image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL});
  ...
  nvvk::cmdImageMemoryBarrier(cmd, {internalImg.image.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL});
...
void ElementInspectorInternal::initComputeInspection(uint32_t index, const ElementInspector::ComputeInspectionInfo& info)
{
  ...
  {
    LOGW(
        "Thread count per block (%d) is not a multiple of warp size (%d) - "
        "the inspector cannot handle incomplete warps. This also means your "
        "shader will have low occupancy. Generally, try to use 128, 256, or "
  ...
  {
    m_alloc->destroyBuffer(var.deviceBuffer);
    m_alloc->destroyBuffer(var.hostBuffer);
    m_alloc->destroyBuffer(var.metadata);
  }
...
void ElementInspectorInternal::deinitComputeInspection(uint32_t index)
{
  ...
  m_alloc->destroyBuffer(inspectedComputeVariable.deviceBuffer);
  m_alloc->destroyBuffer(inspectedComputeVariable.metadata);
}
...
void ElementInspectorInternal::deinitCustomInspection(uint32_t index)
{
  ...
  m_alloc->destroyBuffer(inspectedCustomVariable.deviceBuffer);
  m_alloc->destroyBuffer(inspectedCustomVariable.metadata);
}
...
void ElementInspectorInternal::deinitFragmentInspection(uint32_t index)
{
  ...
  m_alloc->destroyBuffer(inspectedFragmentVariable.deviceBuffer);
  m_alloc->destroyBuffer(inspectedFragmentVariable.metadata);
}
...
std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatRGBA8(const std::string& name /*= ""*/)
{
  if(name.empty())
  {
    return {
...
std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatRGBA32(const std::string& name /*= ""*/)
{
  if(name.empty())
  {
    return {
...
std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatVector4(ValueType type, const std::string& name)
{
  if(name == "")
  {
...
std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatVector3(ValueType type, const std::string& name)
{
  if(name == "")
  {
...
std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatVector2(ValueType type, const std::string& name)
{
  if(name == "")
  {
...
std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatValue(ValueType type, const std::string& name)
{
  return {{type, name}};
}

std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatInt32(const std::string& name /*= "value"*/)
{
  return formatValue(eInt32, name);
}

std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatUint32(const std::string& name /*= "value"*/)
{
  return formatValue(eUint32, name);
}

std::vector<nvapp::ElementInspector::ValueFormat> ElementInspector::formatFloat32(const std::string& name /*= "value"*/)
{
