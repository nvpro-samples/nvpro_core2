...
batches_pooled:  fn (uint64_t itemIndex, uint32_t threadIndex)
                 callback does single item
                 uses at most get_thread_pool().thread_count() threads

ranges_pooled:   fn (uint64_t itemBegin, uint64_t itemEnd, uint32_t threadIndex)
...
batches_pooled and ranges_pooled.
...
template <uint64_t BATCHSIZE = 512, typename F>
inline void parallel_batches_pooled(uint64_t numItems, F&& fn, uint32_t numThreads = 0)
{
...
// Almost the same as the implementation of parallel_batches_pool, but the provided function
// operates on a range of elements explicitly.
template <uint64_t BATCHSIZE = 512, typename F>
inline void parallel_ranges_pooled(uint64_t numItems, F&& fn, uint32_t numThreads = 0)
{
  if(numItems <= BATCHSIZE || numThreads == 1 || BS::this_thread::get_index().has_value())
  {
    fn(0, numItems, 0);
