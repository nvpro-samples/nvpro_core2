...
public:
  ...
  // When set, the filter is called for each key during INI loading.
  // Return true to load the value from INI, false to skip (keep the current value).
  void setLoadFilter(std::function<bool(const std::string& key)> fn) { loadFilter = std::move(fn); }
...
private:
  ...
  std::function<bool(const std::string& key)>   loadFilter;
};
