...
ElementGpuMonitor::ElementGpuMonitor(bool show /*= false*/)
    : showWindow(show)
{
}

void ElementGpuMonitor::onAttach(nvapp::Application* app)
{
...
#endif
  m_settingsHandler.setHandlerName("ElementNvml");
  m_settingsHandler.setSetting("ShowWindow", &showWindow);
  ...
  if(ImGui::Begin("NVML Monitor", &showWindow))
  {
    ...
    for(uint32_t deviceIndex = 0; deviceIndex < m_nvmlMonitor->getGpuCount(); deviceIndex++)
    {
      ...
      else
      {
        if(m_throttleDetected)
        {
          if(m_throttleCooldownTimer.getMilliseconds() > THROTTLE_COOLDOWN_TIME)
          {
            m_throttleDetected = false;
          }
          else
          {
            if(m_throttleCooldownTimer.getMilliseconds() > THROTTLE_SHOW_COOLDOWN_TIME)
            {
            ...
            else
            {
              ImGui::TextColored(ImVec4(1.f, 0.f, 0.f, 1.f),
                                 "Throttle detected for GPU %d: %s - Performance numbers will be unreliable", deviceIndex,
                                 NvmlMonitor::DevicePerformanceState::getThrottleReasonStrings(m_lastThrottleReason)[0].c_str());
            }
          }
        }
...
void ElementGpuMonitor::imguiProgressBars()
{
  ...
  for(uint32_t gpuIndex = 0; gpuIndex < m_nvmlMonitor->getGpuCount(); gpuIndex++)  // Number of gpu
  {
    ...
    if(deviceMemoryInfo.memoryTotal.isSupported == false)
      continue;

    double GiBvalue = 1'000'000'000;
    std::string progtext = fmt::format("{:.2f}/{:.2f} GiB", static_cast<double>(deviceMemoryInfo.memoryUsed.get()[offset]) / GiBvalue,
...
void ElementGpuMonitor::imguiCopyableText(const std::string& text, uint64_t uniqueId)
{
...
void ElementGpuMonitor::imguiDevicePerformanceState(uint32_t deviceIndex)
{
  ...
  plotSize.y =
      std::max(plotSize.y, ImGui::GetTextLineHeight() * NvmlMonitor::DevicePerformanceState::getAllThrottleReasonList().size());
...
void ElementGpuMonitor::imguiClockSetup(uint32_t deviceIndex)
{
  ...
  if(deviceInfo.supportedGraphicsClocks.isSupported && !deviceInfo.supportedGraphicsClocks.get().empty())
  {
    ...
    auto defaultClock = it->second[m_selectedGraphicsClock];
