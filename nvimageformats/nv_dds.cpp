...
#include <directx/dxgiformat.h>  // Included in third_party's dxh subproject
...
static void usage_nv_dds()
{
  ...
  // ErrorWithText is either empty, indicating success, or contains a string
  // with an error message.
  if(maybeError.has_value())
  {
    printf("Error reading data/image.dds: %s\n", maybeError.value().c_str());
    return;
  }

  // Print out some information about the image:
  printf("%s\n", image.formatInfo().c_str());

  // Now we'd normally upload all subresources to the GPU.
  // For this example, we'll show how to access subresources by printing out
  // the first few bytes:
  for(uint32_t mip = 0; mip < image.getNumMips(); mip++)
  {
    for(uint32_t layer = 0; mip < image.getNumLayers(); layer++)
    {
      for(uint32_t face = 0; face < image.getNumFaces(); face++)
      {
        printf("mip %u, layer %u, face %u:\n", mip, layer, face);
        const nv_dds::Subresource& subresource = image.subresource(mip, layer, face);
        printf("%u x %u x %u, %zu bytes\n", image.getWidth(mip), image.getHeight(mip), image.getDepth(mip),
               subresource.data.size());

        printf("data:");
        constexpr size_t kMaxBytesToPrint = 10;
        for(size_t i = 0; i < kMaxBytesToPrint && i < subresource.data.size(); i++)
        {
          printf(" %u", static_cast<uint32_t>(subresource.data[i]));
        }
        if(subresource.data.size() > kMaxBytesToPrint)
        {
          printf("...");
        }
        printf("\n");
  ...
  // We'll make a 16x16 DXGI_FORMAT_RGBA8_UNORM image with 3 mips.
  // Each mip will contain part of a Sierpinski gasket.
  nv_dds::Image outImage;
  outImage.allocate(3 /* numMips */, 1 /* numLayers */, 1 /* numFaces */);
  outImage.mip0Width  = 16;
  outImage.mip0Height = 16;

  for(uint32_t mip = 0; mip < outImage.getNumMips(); mip++)
  {
    // Create image data. This part is arbitrary; for instance, you might use
    // data you generated or loaded from elsewhere. The important part is the
    // calls to outImage.
    const uint32_t       sizeX = outImage.getWidth(mip);
    const uint32_t       sizeY = outImage.getHeight(mip);
    std::vector<uint8_t> data(sizeX * sizeY * 4);

    for(uint32_t y = 0; y < sizeY; y++)
    {
      for(uint32_t x = 0; x < sizeX; x++)
      {
        const uint8_t  gasket         = 255u * !!(x & y);
        const uint32_t firstComponent = (y * sizeX + x) * 4;
        data[firstComponent + 0]      = gasket;
        data[firstComponent + 1]      = gasket;
        data[firstComponent + 2]      = gasket;
        data[firstComponent + 3]      = 255u;
      }
    }

    // Copy it into the subresource:
    maybeError = outImage
                     .subresource(mip, 0, 0)  // mip, layer, frace
                     .create(data.size(), data.data());
    if(maybeError.has_value())
    {
      printf("Error creating subresource: %s\n", maybeError.value().c_str());
      return;
    }
  }

  // Now write the image out:
  nv_dds::WriteSettings writeSettings = {};  // The defaults are OK
  maybeError                          = outImage.writeToFile("example.dds", writeSettings);
  if(maybeError.has_value())
  {
