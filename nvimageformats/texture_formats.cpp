...
struct std::hash<texture_formats::OpenGLFormat>
{
  ...
  {
    // TODO: Switch to a better hash combiner! This one is not good because
    // std::hash<integral>()(x) is equal to x in libstdc++.
...
// This uses the X macro (https://en.wikipedia.org/wiki/X_macro) strategy, so
// we only have to define this table once and then can derive all the
// format conversion and stringification functions.
//
// But since multiple OpenGL formats can map to the same DXGI or Vulkan format:
// - REPVK will be called for repeated Vulkan formats
// - REPBOTH will be called if both repeat
// - We need to build some tables at runtime to avoid duplicate values.
